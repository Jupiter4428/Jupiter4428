# Hi there, I'm Wutthisak 👋

### 👨‍💻 Database & Backend Developer | Backend Architect | Documentation Enthusiast
I'm a student developer focused on building scalable backend systems, creating detailed technical documentation, and exploring the intersection of finance and technology.

---

### 🚀 Technical Skills
* **Languages:** Java, Python, HTML, SQL
* **Frameworks & Tools:** Flask, MySQL
* **Documentation & Design:** Technical Writing, API Documentation, System Flowcharts, Database Schema Design
* **Architecture:** Database Design, Backend Architecture

### ✍️ Why I Love Documentation
I believe that great code is only half of the story. I enjoy:
* **Structured Thinking:** การถ่ายทอดระบบที่ซับซ้อนให้เข้าใจง่ายผ่านเอกสารและแผนภาพ
* **Detail-Oriented:** การจดบันทึกลำดับการทำงาน (Workflow) เพื่อให้การทำงานในทีมราบรื่นที่สุด
* **Quality First:** การทำ API Spec และ README ที่ชัดเจน เพื่อให้โปรเจกต์ยั่งยืนและต่อยอดได้ง่าย

### 📊 Trading & Financial Interests
I have a strong interest in financial market analysis and automated trading:
* **Strategies:** Smart Money Concepts (SMC), Technical Analysis (RSI, MACD, FVG)
* **Markets:** Gold (XAUUSD), Bitcoin (BTC), and US Tech Stocks (TSM, ASML)

### 📁 Featured Projects
* **Tutor Match:** A platform connecting students and tutors (Flask & MySQL)
---

### 📫 Connect with me:
* **Email:** wutthisak2548@gmail.com
* **Instagram:** m_m3nz

![Your GitHub Stats](https://github-readme-stats.vercel.app/api?username=Jupiter4428&show_icons=true&theme=dark)
